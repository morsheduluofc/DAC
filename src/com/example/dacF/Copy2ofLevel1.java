...
public class Copy2ofLevel1 extends Activity implements OnTouchListener, OnClickListener,SensorEventListener {
  ...
  public void onCreate(Bundle savedInstanceState) {
   ...
   // circlePaint.setStrokeJoin(Paint.Join.MITER);
    ...
    AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(context);
		alertDialogBuilder.setTitle("Complete all challenges.");
		// set dialog message
		alertDialogBuilder
			.setMessage("Click yes to go home.")
			.setCancelable(false)
		...
		AlertDialog.Builder alertDialogBuilder1 = new AlertDialog.Builder(context);
			...
			// set dialog message
			alertDialogBuilder1
				.setMessage("Click yes to start from round 1")
				.setCancelable(false)
				.setPositiveButton("Yes",new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog,int id) {
						// if this button is clicked, close
	  ...
	  case R.id.l2next_button:
		  if(threeCircle){
			  if(radi==1){
					...
					out="Round:"+(round/4+1)+". Maximum number of tries: 30";	
					}
			  text.setText(out);
			  tryaFlasedCirlce();
...
//	  	if(radi==1 || radi==2){
//		  CombineAllFeatures();
//		  new MyAsyncTask().execute(Confirm1.fname);
//		  Confirm1.fname="";
//		  }
	  	  finish();
		  break;
...
/*--------------------Send data-----------------------------------*/  
  private class MyAsyncTask extends AsyncTask<String, Integer, Double>{
		...
		public void postData(String valueIWantToSend) {
			...
			String message="";
			message=Copy2ofLevel1.fStore;
			message=message+Copy2ofLevel1.store1;
			//message=message+Level2.store2;
			Copy2ofLevel1.fStore="";
			Copy2ofLevel1.store1="";
			Level2.store2="";
...
//  	    row=0;
  ...
  void MissCirelce(boolean miss){
  	Runnable myRunnable = new Runnable() {
  	public void run() {
  		if(threeCircle==true){
  			callMe();
//  			Toast.makeText(Level1.this, "Miss the suggestion? Tap on the screen.", Toast.LENGTH_LONG).show();
//	    	out="Miss the suggestion? Tap on the screen.";
// 	        text.setText(out);   	       
// 	        missflashed=true;
  		}
  	}
  	...
  	if(miss){
  	handler1.postDelayed(myRunnable,5000);
	  }
	  if(!miss){
  ...
  public void callMe(){
	  if(notmissflashed==false && ftime==0){
			Toast.makeText(Copy2ofLevel1.this, "Miss the suggestion? Tap on the screen.", Toast.LENGTH_LONG).show();
	  }
	  if(notmissflashed==false){
	    	out="Miss the suggestion? Tap on the screen.";
	        text.setText(out);   	       
	        missflashed=true;
  }
  ...
  void validation(){
	  for(int j=0;j<row;j++){
		  ...
		  if(xbound<0 ||xbound>360 ||ybound<30 ||ybound>540){
			  Toast.makeText(Copy2ofLevel1.this, "Drawing is out of boundary", Toast.LENGTH_SHORT).show();
			  //tryagain();
			  drawAgain();
		  }
	  }
...
//       }else 
       ...
       if(stindication==1){
       Toast.makeText(Copy2ofLevel1.this, "Erro in starting point. Try again", Toast.LENGTH_SHORT).show();
       stindication=0;
       }else if(timeindication==1){
    	Toast.makeText(Copy2ofLevel1.this, "Too long time. Try again", Toast.LENGTH_SHORT).show();
    	timeindication=0;
       }else{
 	   Toast.makeText(Copy2ofLevel1.this, "Not close to the circle. Try again", Toast.LENGTH_SHORT).show();
       }
...
/*--------------------- Start 1----------------------------*/
   public boolean onTouch(View v, MotionEvent event) {
       ...
       if(drawenable==0){
    	   Toast.makeText(Copy2ofLevel1.this, "Wait!!", Toast.LENGTH_SHORT).show();
    	   return false;
       }else{
//    	   if(!missflashed){
//    		   notmissflashed=true;
